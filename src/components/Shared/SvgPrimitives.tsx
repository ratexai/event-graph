    ...
    <g opacity={isDimmed ? 0.04 : 1} style={{ transition: "opacity 0.3s" }}>
      <defs>
        <linearGradient id={gradId} x1="0%" y1="0%" x2="100%" y2="0%">
          <stop offset="0%" stopColor={fromColor} stopOpacity={isActive ? 0.5 : 0.08} />
          <stop offset="100%" stopColor={toColor} stopOpacity={isActive ? 0.6 : 0.15} />
        </linearGradient>
      </defs>
...
export const GridColumn = memo<GridColumnProps>(({ x, topY, bottomY, label, theme }) => (
  <g>
    <line x1={x} y1={topY - 14} x2={x} y2={bottomY + 14} stroke={theme.border} strokeWidth={1} strokeDasharray="1 6" opacity={0.5} />
    <rect x={x - 24} y={bottomY + 18} width={48} height={18} rx={9} fill={theme.bgAlt} stroke={theme.border} strokeWidth={0.5} />
    <text x={x} y={bottomY + 30} textAnchor="middle" fill={theme.muted} fontSize={8.5} fontFamily="'JetBrains Mono',monospace" fontWeight={500}>{label}</text>
  </g>
));
